...
const pdfSnapshot = computed<CharacterPdfSnapshot>(() => ({
  ...
  level: model.level ?? 1,
  daseinsform: model.daseinsform ?? "",
  portrait: model.portrait ?? null,
  st: model.st ?? 10,
  dx: model.dx ?? 10,
  iq: model.iq ?? 10,
  ht: model.ht ?? 10,
...
<template>
  <div class="min-h-screen bg-base-300 pb-16 print:bg-white">
    <template v-if="chars.workingCopy">
      ...
      <div v-if="pdfError" class="max-w-2xl mx-auto px-4 pt-3 print:hidden">
        ...
        >
          ...
          <div class="bg-base-200 px-6 pt-6 pb-4 border-b border-base-300">
            <div class="flex items-start justify-between gap-4">
              <h1
                class="text-2xl font-bold tracking-tight leading-tight min-w-0 truncate"
              >
                {{ chars.workingCopy.name || "Unbenannt" }}
              </h1>
              <div class="flex items-baseline gap-1 shrink-0">
                <span
                  class="text-xs text-base-content/50 uppercase tracking-wide"
                  >Stufe</span
                >
          ...
          <div class="px-6 py-5 flex flex-col gap-6">
            ...
            <section>
              <h2
                class="text-xs font-bold uppercase tracking-widest text-base-content/40 mb-2 border-b border-base-200 pb-1"
              >
                Attribute
              </h2>
              <!-- Primärattribute -->
            ...
            <section v-if="activeSkills.length">
              ...
              <table class="w-full border-collapse">
                ...
                <tbody>
                  ...
                  >
                    <td class="list-td text-sm font-medium">
                      {{ skill.name }}
                    </td>
                    <td class="list-td text-sm font-bold font-mono text-right">
                      {{ skill.currentLevel }}
                    </td>
                  </tr>
                </tbody>
                ...
                <tbody>
                  <template v-for="trait in model.traits" :key="trait.name">
                    ...
                    <tr v-if="trait.description" class="border-base-200">
                      <td
                        class="list-td-desc text-xs text-base-content/60 whitespace-pre-wrap"
                      >
                        {{ trait.description }}
                      </td>
                    </tr>
