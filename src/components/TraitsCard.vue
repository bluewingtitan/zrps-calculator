...
const totalCp = computed(() =>
  store.traits.reduce((sum, t) => sum + (t.cp ?? 0), 0),
);

function addTrait() {
  store.traits.push({ name: "", cp: 0 });
}

function removeTrait(idx: number) {
  store.traits.splice(idx, 1);
}

function onCpInput(idx: number, event: Event) {
  const raw = (event.target as HTMLInputElement).value;
  const parsed = parseInt(raw, 10);
  const trait = store.traits[idx];
  if (!trait) return;
  trait.cp = isNaN(parsed) ? 0 : parsed;
}
</script>
  ...
  <div class="card bg-base-200 shadow">
    <div class="card-body gap-0 p-4">
      <div class="flex items-center justify-between mb-3">
        <h2 class="card-title text-base">Vor- &amp; Nachteile</h2>
        <span
      ...
      <div class="divide-y divide-base-300">
        <div
          v-for="(trait, idx) in store.traits"
          :key="idx"
      ...
      <!-- Empty state -->
      <p
        v-if="store.traits.length === 0"
        class="text-xs text-base-content/40 text-center py-3"
      >
        Noch keine Vor- oder Nachteile hinzugefügt.
      </p>
